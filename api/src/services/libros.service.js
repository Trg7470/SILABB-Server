...
static async obtenerPorId(idLibro) {
    ...
    const libro = await LibrosModel.obtenerPorId(idLibro);
...
static async buscar(termino) {
    ...
    return await LibrosModel.buscar(termino.trim());
}
...
static async verificarDisponible(idLibro) {
    ...
    return await LibrosModel.verificarDisponible(idLibro);
}
...
static async crear(data, idUsuario) {
    ...
    if (ISBN) {
        const libros = await LibrosModel.buscar(ISBN.trim());

        const existeISBN = libros.some(
            libro =>
    ...
    const idLibro = await LibrosModel.crear(
        Titulo.trim(),
    ...
    return await LibrosModel.obtenerPorId(idLibro);
}

static async actualizar(idLibro, data, idUsuario) {
    const {
        Titulo,
        Autor,
        Editorial,
    ...
    const resultado = await LibrosModel.actualizar(
        idLibro,
        Titulo.trim(),
        Autor ? Autor.trim() : null,
        Editorial ? Editorial.trim() : null,
...
static async cambiarEstado(idLibro, activo, idUsuario) {
    if (!idLibro) {
        throw new Error('El libro es obligatorio');
    }
    ...
    if (!activo) {
        const tienePrestamoActivo =
            await PrestamosModel.tienePrestamoActivo(idLibro);
...
static async eliminar(idLibro, idUsuario) {
    if (!idLibro) {
        throw new Error('El libro es obligatorio');
    }
    ...
    if (!idUsuario) {
        throw new Error(
            'El usuario que elimina el libro es obligatorio'
        );
    }
    ...
    if (tienePrestamoActivo) {
        throw new Error(
            'No se puede eliminar un libro que tiene un préstamo activo'
        );
    }

    const resultado =
        await LibrosModel.eliminar(idLibro);
